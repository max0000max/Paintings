...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateSlider(animate = true) {
    ...
    track.style.transform = `translate3d(-${getSlideOffset(currentIndex)}px, 0, 0)`;
  }
  ...
  function nextSlide() {
    ...
    updateSlider();
    // Перезапускаем таймер после любого изменения слайда
    startAutoplay();
  }
  ...
  slider.addEventListener('pointermove', (event) => {
    ...
    track.style.transform = `translate3d(${-dragOffset + distance}px, 0, 0)`;
  });
  ...
  if (btnNext) {
    btnNext.addEventListener('click', (e) => {
      ...
      e.stopPropagation();
      nextSlide();
      // Явно перезапускаем таймер при клике
      startAutoplay();
    });
  }
